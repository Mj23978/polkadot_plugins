...
class Keyring {
  ...
  KeyPair get current {
    ...
    if (list.length > 0) {
      ...
      return i >= 0 ? list[i] : KeyPair();
    }
    return KeyPair();
  }

  void setCurrent(KeyPair acc) {
    store.setCurrentPubKey(acc.pubKey);
  }

  List<KeyPair> get keyPairs {
    return store.list.map((e) => KeyPair.fromJson(e)).toList();
  }

  List<KeyPair> get externals {
    return store.externals.map((e) => KeyPair.fromJson(e)).toList();
  }

  List<KeyPair> get contacts {
    return store.contacts.map((e) => KeyPair.fromJson(e)).toList();
  }

  List<KeyPair> get allAccounts {
    final res = keyPairs;
    res.addAll(externals);
    return res;
  }

  List<KeyPair> get allWithContacts {
    final res = keyPairs;
    res.addAll(contacts);
    return res;
  }

  List<KeyPair> get optionals {
