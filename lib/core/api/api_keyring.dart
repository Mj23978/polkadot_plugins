...
import '../../models/verify_result/verify_result.dart';
import '../service/keyring.dart';
...
class ApiKeyring {
  ApiKeyring(this.apiRoot) : this.service = ServiceKeyring(apiRoot.service);
  ...
  Future<KeyPair> addAccount(
    ...
    apiRoot.updateIndicesMap(keyring, [acc['address']]);

    return KeyPair.fromJson(acc as Map<String, dynamic>);
  }

  /// Add a contact.
  Future<KeyPair> addContact(Keyring keyring, Map acc) async {
    final pubKey = await (apiRoot.service.webView!
            .evalJavascript('account.decodeAddress(${jsonEncode([acc['address']])})') as FutureOr<Map<dynamic, dynamic>>);
  ...
  Future<SeedBackup?> getDecryptedSeed(Keyring keyring, password) async {
    final Map? data =
    ...
    return SeedBackup.fromJson(data as Map<String, dynamic>);
  }

  /// delete account from storage
  Future<void> deleteAccount(Keyring keyring, KeyPair account) async {
    if (account != null) {
  ...
  Future<bool> checkPassword(KeyPair account, String pass) async {
    final res = await service.checkPassword(account.pubKey, pass);
    return res;
  }

  /// change password of account
  Future<KeyPair?> changePassword(
    ...
    return KeyPair.fromJson(res as Map<String, dynamic>);
  }

  /// change name of account
  Future<KeyPair> changeName(Keyring keyring, String name) async {
  ...
  Future<ExtensionSignResult?> signAsExtension(
    ...
    return res.copyWith(id: param.id, signature: signature['signature']);
  }
