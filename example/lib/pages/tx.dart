...
import 'package:polkadot_plugins/models/tx_data/tx_data.dart';

import 'keyring.dart';

class TxPage extends StatefulWidget {
  TxPage(this.showResult);
...
class _TxPageState extends State<TxPage> {
  ...
  Future<void> _estimateTxFee() async {
    ...
    final sender = TxSender(address: _testAddress, pubKey: _testPubKey);
    final txInfo = TxInfo(module: 'balances', call: 'transfer', sender: sender);
    final res = await api.tx.estimateFees(txInfo, [
      // params.to
      _testAddressGav,
      // params.amount
      '10000000000'
  ...
  Future<void> _sendTx() async {
    ...
    final sender = TxSender(
      address: keyring.keyPairs[0].address,
      pubKey: keyring.keyPairs[0].pubKey,
    );
    final txInfo = TxInfo(module: 'balances',call:  'transfer',sender:  sender);
    try {
      final hash = await api.tx.signAndSend(
        txInfo,
        [
          // params.to
          // _testAddressGav,
