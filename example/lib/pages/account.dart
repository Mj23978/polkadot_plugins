...
import 'package:polkadot_plugins/models/balance/balance.dart';
import 'package:polkadot_plugins_example/pages/keyring.dart';

class AccountPage extends StatefulWidget {
  AccountPage(this.showResult);
...
class _AccountPageState extends State<AccountPage> {
  ...
  Future<void> _encodeAddress() async {
    ...
    final Map? res = await api.account.encodeAddress([_testPubKey]);
    widget.showResult(
      context,
      'encodeAddress',
  ...
  Future<void> _decodeAddress() async {
    ...
    final Map? res = await api.account.decodeAddress([_testAddress]);
    widget.showResult(
      context,
      'decodeAddress',
      JsonEncoder.withIndent('  ').convert(res),
  ...
  Future<void> _getPubKeyIcons() async {
    ...
    final List? res = await api.account.getPubKeyIcons([_testPubKey]);
    widget.showResult(
  ...
  Future<void> _getAddressIcons() async {
    ...
    final List? res =
        await  api.account.getAddressIcons([_testAddress]);
    widget.showResult(
  ...
  Future<void> _queryIndexInfo() async {
    ...
    final List? res = await api.account.apiRoot
  ...
  Future<void> _queryBalance() async {
    ...
    final res = await api.account.queryBalance(_testAddress);
  ...
  Future<void> _subscribeBalance() async {
    final channel =
        await api.account.subscribeBalance(_testAddress, (res) {
      setState(() {
        _balance = res;
      });
    });
  ...
  Future<void> _checkAddressFormat() async {
    ...
    final res =
        await api.account.checkAddressFormat(_testAddress, 2);
  ...
  void dispose() {
    if (_msgChannel != null) {
       api.account.apiRoot.unsubscribeMessage(_msgChannel!);
    }
    super.dispose();
  }
